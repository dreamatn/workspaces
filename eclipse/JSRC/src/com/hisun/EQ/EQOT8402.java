...
public class EQOT8402 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "EQOT8402 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.N_EXP_DT);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.C_SPR_NM);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.C_SPR_NO);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.CONT_RSN);
        ...
        if (EQB0004_AWA_0004.N_EXP_DT == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_NEXPDT_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.N_EXP_DT_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        } else {
            if (EQB0004_AWA_0004.N_EXP_DT <= EQB0004_AWA_0004.EXP_DT) {
                WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_EXPDT_ERROR;
                WS_FLD_NO = EQB0004_AWA_0004.N_EXP_DT_NO;
                S000_ERR_MSG_PROC_CONTINUE();
                if (pgmRtn) return;
            }
        }
        if (EQB0004_AWA_0004.C_SPR_NM.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_SPRNM_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.C_SPR_NM_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        } else {
            if (!EQB0004_AWA_0004.C_SPR_NM.equalsIgnoreCase(EQB0004_AWA_0004.SPR_NM)) {
                WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_CSPRNM_ERROR;
                WS_FLD_NO = EQB0004_AWA_0004.C_SPR_NM_NO;
                S000_ERR_MSG_PROC_CONTINUE();
                if (pgmRtn) return;
            }
        }
        if (EQB0004_AWA_0004.C_SPR_NO.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_SPRNO_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.C_SPR_NO_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
        if (EQB0004_AWA_0004.CONT_RSN.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_CORSN_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.CONT_RSN_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
    ...
    public void B020_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        EQCSFRZ.DATA.N_EXP_DT = EQB0004_AWA_0004.N_EXP_DT;
        EQCSFRZ.DATA.C_SPR_NM = EQB0004_AWA_0004.C_SPR_NM;
        EQCSFRZ.DATA.C_SPR_NO = EQB0004_AWA_0004.C_SPR_NO;
        EQCSFRZ.DATA.CONT_RSN = EQB0004_AWA_0004.CONT_RSN;
