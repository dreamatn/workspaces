...
public class EQOT8302 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "EQOT8302 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        ...
        if (EQB0003_AWA_0003.PLG_NO.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_PLG_NO_MUST_INPUT;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        if (EQB0003_AWA_0003.REL_QTY <= 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_REL_QTY_INVALID;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        if (EQB0003_AWA_0003.REL_NUM.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_REL_NUM_MUST_INPUT;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
    }
    public void B020_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        EQCMPPLG.DATA.PLG_NO = EQB0003_AWA_0003.PLG_NO;
        EQCMPPLG.DATA.REL_QTY = EQB0003_AWA_0003.REL_QTY;
        EQCMPPLG.DATA.REL_NUM = EQB0003_AWA_0003.REL_NUM;
