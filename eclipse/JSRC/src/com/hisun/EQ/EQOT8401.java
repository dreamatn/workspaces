...
public class EQOT8401 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "EQOT8401 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.NOW_QTY);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.NFRZ_QTY);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.NPLG_QTY);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.SPR_NO);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.SPR_NM);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.FRZ_QTY);
        CEP.TRC(SCCGWA, EQB0004_AWA_0004.RELSE_DT);
        ...
        if (EQB0004_AWA_0004.SPR_NO.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_SPRNO_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.SPR_NO_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
        if (EQB0004_AWA_0004.SPR_NM.trim().length() == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_SPRNM_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.SPR_NM_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
        if (EQB0004_AWA_0004.FRZ_QTY == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_FRZQTY_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.FRZ_QTY_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        } else {
            if (EQB0004_AWA_0004.FRZ_QTY <= 0) {
                WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_FRZQTY_ERROR;
                WS_FLD_NO = EQB0004_AWA_0004.FRZ_QTY_NO;
                S000_ERR_MSG_PROC_CONTINUE();
                if (pgmRtn) return;
            }
        }
        if (EQB0004_AWA_0004.RELSE_DT == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_RELDT_MUST_INPUT;
            WS_FLD_NO = EQB0004_AWA_0004.RELSE_DT_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        } else {
            if (EQB0004_AWA_0004.RELSE_DT < SCCGWA.COMM_AREA.AC_DATE) {
                WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_RELDT_ERROR;
                WS_FLD_NO = EQB0004_AWA_0004.RELSE_DT_NO;
                S000_ERR_MSG_PROC_CONTINUE();
                if (pgmRtn) return;
            }
    ...
    public void B020_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        EQCSFRZ.DATA.PSBK_NO = EQB0004_AWA_0004.PSBK_NO;
        EQCSFRZ.DATA.SPR_NO = EQB0004_AWA_0004.SPR_NO;
        EQCSFRZ.DATA.SPR_NM = EQB0004_AWA_0004.SPR_NM;
        EQCSFRZ.DATA.FRZ_QTY = EQB0004_AWA_0004.FRZ_QTY;
        EQCSFRZ.DATA.RELSE_DT = EQB0004_AWA_0004.RELSE_DT;
