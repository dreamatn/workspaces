...
public class EQOT8505 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "EQOT8505 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, EQB1100_AWA_1100.START_DT);
        CEP.TRC(SCCGWA, EQB1100_AWA_1100.FIN_DT);
        ...
        if (EQB1100_AWA_1100.START_DT == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_STARTDT_MUST_INPUT;
            WS_FLD_NO = EQB1100_AWA_1100.START_DT_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
        if (EQB1100_AWA_1100.FIN_DT == 0) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_FINDT_MUST_INPUT;
            WS_FLD_NO = EQB1100_AWA_1100.FIN_DT_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
        if (EQB1100_AWA_1100.FIN_DT < EQB1100_AWA_1100.START_DT) {
            WS_ERR_MSG = EQCMSG_ERROR_MSG.EQ_FINDT_MUST_BIGGER;
            WS_FLD_NO = EQB1100_AWA_1100.FIN_DT_NO;
            S000_ERR_MSG_PROC_CONTINUE();
            if (pgmRtn) return;
        }
    ...
    public void B020_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        EQCSINFO.DATA.START_DT = EQB1100_AWA_1100.START_DT;
        EQCSINFO.DATA.FIN_DT = EQB1100_AWA_1100.FIN_DT;
        EQCSINFO.FUNC = 'D';
        S000_CALL_EQZSINFO();
        if (pgmRtn) return;
    }
