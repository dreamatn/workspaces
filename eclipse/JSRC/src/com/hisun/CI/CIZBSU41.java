...
public class CIZBSU41 {
    ...
    CIRBSP41 CIRBSP41 = new CIRBSP41();
    SCCGWA SCCGWA;
    ...
    public void MP(SCCGWA SCCGWA, SCCRWBSP SCCRWBSP) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "CIZBSU41 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        CIRLSP41 = (PSRBSP01) SCCRWBSP.INFO.PTR;
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
        IBS.init(SCCGWA, CIRBSP41);
        IBS.CLONE(SCCGWA, CIRLSP41, CIRBSP41);
    ...
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        IBS.CLONE(SCCGWA, CIRBSP41, CIRLSP41);
    }
    public void B010_CREATE_RECORD_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, CIRBSP41);
        T000_WRITE_BSP();
        if (pgmRtn) return;
    ...
    public void T000_READ_BSP() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, CIRBSP41.KEY.AP_TYPE);
        CEP.TRC(SCCGWA, CIRBSP41.KEY.AP_BATNO);
        CEP.TRC(SCCGWA, CIRBSP41.KEY.BAT_NO);
        CITBSP41_RD = new DBParm();
        CITBSP41_RD.TableName = "CITBSP41";
        IBS.READ(SCCGWA, CIRBSP41, CITBSP41_RD);
    ...
    public void T000_WRITE_BSP() throws IOException,SQLException,Exception {
        CITBSP41_RD = new DBParm();
        CITBSP41_RD.TableName = "CITBSP41";
        CITBSP41_RD.errhdl = true;
        IBS.WRITE(SCCGWA, CIRBSP41, CITBSP41_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_READ_BSP_UPD() throws IOException,SQLException,Exception {
        ...
        CITBSP41_RD = new DBParm();
        CITBSP41_RD.TableName = "CITBSP41";
        CITBSP41_RD.upd = true;
        CITBSP41_RD.col = "STATUS, RT_CODE, OUT_JRN, OUT_VCH_NO, OUT_DATA, TS";
        IBS.READ(SCCGWA, CIRBSP41, CITBSP41_RD);
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.DBIO_FLG);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_REWRITE_BSP() throws IOException,SQLException,Exception {
        CITBSP41_RD = new DBParm();
        CITBSP41_RD.TableName = "CITBSP41";
        IBS.REWRITE(SCCGWA, CIRBSP41, CITBSP41_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
            IBS.CPY2CLS(SCCGWA, CICMSG_ERROR_MSG.CI_SYS_ERROR, SCCRWBSP.RC);
    ...
    public void T000_DELETE_BSP() throws IOException,SQLException,Exception {
        CITBSP41_RD = new DBParm();
        CITBSP41_RD.TableName = "CITBSP41";
        IBS.DELETE(SCCGWA, CIRBSP41, CITBSP41_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
            SCCRWBSP.RETURN_INFO = 'N';
