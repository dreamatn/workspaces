...
public class CIOT5800 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "CIOT5800 return!");
        Z_RET();
    }
    ...
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        if (CICMLS2.DATA.LST_CD.equalsIgnoreCase("WHT")) {
        ...
        CEP.TRC(SCCGWA, CICMLS2.DATA.CI_ENM);
        CEP.TRC(SCCGWA, CICMLS2.DATA.CI_CNM);
        CICMLS2.FUNC = 'B';
        if (CICMLS2.DATA.ID_TYPE.trim().length() == 0 
            && CICMLS2.DATA.ID_NO.trim().length() == 0) {
            CICMLS2.OPT = 'L';
        }
        if (CICMLS2.DATA.ID_TYPE.trim().length() > 0 
            && CICMLS2.DATA.ID_NO.trim().length() > 0) {
