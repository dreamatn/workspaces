...
public class SOZBGRP {
    brParm SOTGRP_BR = new brParm();
    int JIBS_tmp_int;
    ...
    short WS_LEN = 0;
    String WS_TSQ_REC = " ";
    SOZBGRP_WS_TS_CNTL WS_TS_CNTL = new SOZBGRP_WS_TS_CNTL();
    SOZBGRP_WS_LINE WS_LINE = new SOZBGRP_WS_LINE();
    SOZBGRP_WS_SUB_TIT WS_SUB_TIT = new SOZBGRP_WS_SUB_TIT();
    SOZBGRP_WS_REC_OUT WS_REC_OUT = new SOZBGRP_WS_REC_OUT();
    char WS_FND_FLG = ' ';
    ...
    public void T01_STARTBR_SOTGRP() throws IOException,SQLException,Exception {
        IBS.init(SCCGWA, SORGRP);
        WS_GRP_ID = SOCIGRP.ID;
        SOTGRP_BR.rp = new DBParm();
        SOTGRP_BR.rp.TableName = "SOTGRP";
        SOTGRP_BR.rp.where = "ID >= :WS_GRP_ID";
        SOTGRP_BR.rp.errhdl = true;
        SOTGRP_BR.rp.order = "BANK_NO,ID";
        IBS.STARTBR(SCCGWA, SORGRP, this, SOTGRP_BR);
    ...
    public void T02_READNEXT_SOTGRP() throws IOException,SQLException,Exception {
        SOTGRP_BR.rp.errhdl = true;
        IBS.READNEXT(SCCGWA, SORGRP, this, SOTGRP_BR);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            WS_FND_FLG = 'Y';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
            WS_FND_FLG = 'N';
    ...
    public void T03_ENDBR_SOTGRP() throws IOException,SQLException,Exception {
        IBS.ENDBR(SCCGWA, SOTGRP_BR);
    }
    public void B02_PROC_PRM() throws IOException,SQLException,Exception {
        WS_REC_OUT.WS_ID = SORGRP.KEY.ID;
        WS_REC_OUT.WS_NAME = SORGRP.NAME;
        WS_REC_OUT.WS_SERV_ID = SORGRP.SERV_ID;
        WS_REC_OUT.WS_CRT_USER = SORGRP.CRT_USER;
        if (SORGRP.TS == null) SORGRP.TS = "";
        JIBS_tmp_int = SORGRP.TS.length();
        for (int i=0;i<26-JIBS_tmp_int;i++) SORGRP.TS += " ";
        JIBS_tmp_str[0] = "" + WS_REC_OUT.WS_CRT_DATE;
        JIBS_f0 = "";
        for (int i=0;i<8-JIBS_tmp_str[0].length();i++) JIBS_f0 += "0";
        JIBS_NumStr = JIBS_f0 + WS_REC_OUT.WS_CRT_DATE;
        JIBS_NumStr = SORGRP.TS.substring(0, 4) + JIBS_NumStr.substring(4);
        WS_REC_OUT.WS_CRT_DATE = Integer.parseInt(JIBS_NumStr);
        if (SORGRP.TS == null) SORGRP.TS = "";
        JIBS_tmp_int = SORGRP.TS.length();
        for (int i=0;i<26-JIBS_tmp_int;i++) SORGRP.TS += " ";
        JIBS_tmp_str[0] = "" + WS_REC_OUT.WS_CRT_DATE;
