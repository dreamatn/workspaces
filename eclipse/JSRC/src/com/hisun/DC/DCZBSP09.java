...
public class DCZBSP09 {
    ...
    DCRBSP09 DCRBSP09 = new DCRBSP09();
    SCCGWA SCCGWA;
    ...
    public void MP(SCCGWA SCCGWA, SCCRWBSP SCCRWBSP) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "DCZBSP09 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DCRBSP09);
        JIBS_tmp_str[1] = JIBS_tmp_str[0];
        IBS.CLONE(SCCGWA, DCRBSP09, DCRBSP1);
    }
    public void B010_CREATE_RECORD_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, DCRBSP09);
        T000_WRITE_BSP();
        if (pgmRtn) return;
    ...
    public void T000_READ_BSP() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, DCRBSP09.KEY.AP_TYPE);
        DCTBSP09_RD = new DBParm();
        DCTBSP09_RD.TableName = "DCTBSP09";
        IBS.READ(SCCGWA, DCRBSP09, DCTBSP09_RD);
    ...
    public void T000_WRITE_BSP() throws IOException,SQLException,Exception {
        DCTBSP09_RD = new DBParm();
        DCTBSP09_RD.TableName = "DCTBSP09";
        DCTBSP09_RD.errhdl = true;
        IBS.WRITE(SCCGWA, DCRBSP09, DCTBSP09_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_READ_BSP_UPD() throws IOException,SQLException,Exception {
        DCTBSP09_RD = new DBParm();
        DCTBSP09_RD.TableName = "DCTBSP09";
        DCTBSP09_RD.upd = true;
        DCTBSP09_RD.col = "STATUS, RT_CODE, OUT_JRN, OUT_VCH_NO, OUT_DATA, TS";
        IBS.READ(SCCGWA, DCRBSP09, DCTBSP09_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
    ...
    public void T000_REWRITE_BSP() throws IOException,SQLException,Exception {
        DCTBSP09_RD = new DBParm();
        DCTBSP09_RD.TableName = "DCTBSP09";
        IBS.REWRITE(SCCGWA, DCRBSP09, DCTBSP09_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
            IBS.CPY2CLS(SCCGWA, DCCMSG_ERROR_MSG.DC_SYS_ERROR, SCCRWBSP.RC);
    ...
    public void T000_DELETE_BSP() throws IOException,SQLException,Exception {
        DCTBSP09_RD = new DBParm();
        DCTBSP09_RD.TableName = "DCTBSP09";
        IBS.DELETE(SCCGWA, DCRBSP09, DCTBSP09_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
            SCCRWBSP.RETURN_INFO = 'N';
