...
public class DCZURHLD {
    ...
    public void MP(SCCGWA SCCGWA, DCCURHLD DCCURHLD) throws IOException,SQLException,Exception {
        this.SCCGWA = SCCGWA;
        this.DCCURHLD = DCCURHLD;
        CEP.TRC(SCCGWA);
        A000_INIT_PROC();
        if (pgmRtn) return;
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "DCZURHLD return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B020_INQ_HLD_INF_PROC() throws IOException,SQLException,Exception {
        ...
        if ((SCCGWA.COMM_AREA.CANCEL_IND != 'Y') 
            ...
            if (DCCURHLD.DATA.RHLD_TYP == ' ' 
                && DCCURHLD.DATA.RAMT > DDRHLD.HLD_AMT) {
                WS_ERR_MSG = DCCMSG_ERROR_MSG.DC_REL_AMT_GT_HLD_AMT;
                S000_ERR_MSG_PROC();
                if (pgmRtn) return;
            }
            if (DCCURHLD.DATA.RHLD_TYP == ' ') {
                if (DCCURHLD.DATA.RAMT == DDRHLD.HLD_AMT) {
                    DCCURHLD.DATA.RHLD_TYP = '1';
                } else {
    ...
    public void B020_CHK_AC_STS_PROC() throws IOException,SQLException,Exception {
        ...
        if (CICQACAC.O_DATA.O_ACAC_DATA.O_FRM_APP_ACAC.equalsIgnoreCase("DD")) {
            ...
            CEP.TRC(SCCGWA, WS_CANCEL_AVL_AMT);
            if (DDRHLD.HLD_STS == 'W') {
    ...
    public void B030_UPDATE_TDTSMST() throws IOException,SQLException,Exception {
        ...
        if (WS_HLD_TYP == '2' 
            && WS_HLD_STS == 'N') {
            TDRSMST.HBAL = TDRSMST.HBAL - DCCURHLD.DATA.RAMT;
        }
        CEP.TRC(SCCGWA, TDRSMST.HBAL);
        R000_CCY_SMST_PROC();
        if (pgmRtn) return;
    ...
    public void B050_NON_FIN_HIS_PROC() throws IOException,SQLException,Exception {
        ...
        if (DCCURHLD.DATA.TX_TYP_CD.trim().length() > 0) {
            BPCPNHIS.INFO.TX_TYP_CD = DCCURHLD.DATA.TX_TYP_CD;
        } else {
    ...
    public void B070_TRANS_DATA_BACK() throws IOException,SQLException,Exception {
        DCCURHLD.DATA.AC = CICQACAC.O_DATA.O_ACR_DATA.O_AGR_NO;
        if (DCCURHLD.DATA.RHLD_TYP == '1') {
            DCCURHLD.DATA.RAMT = WS_BEF_HLD_AMT;
        }
        DCCURHLD.DATA.HLD_TYP = WS_HLD_TYP;
        DCCURHLD.DATA.SPR_TYP = WS_SPR_TYP;
        DCCURHLD.DATA.CCY = WS_CCY;
        DCCURHLD.DATA.CCY_TYP = WS_CCY_TYP;
        DCCURHLD.DATA.BEF_AMT = WS_BEF_HLD_AMT;
        DCCURHLD.DATA.EFF_DT = WS_EFF_DATE;
        DCCURHLD.DATA.EXP_DT = WS_EXP_DATE;
        DCCURHLD.DATA.REM_AMT = WS_REM_HLD_AMT;
        DCCURHLD.DATA.AC_TYPE = WS_AC_TYPE;
        DCCURHLD.DATA.CURR_BAL = WS_CURR_BAL;
        DCCURHLD.DATA.AVL_AMT = WS_AVL_AMT;
        DCCURHLD.DATA.ACC_HLD_AMT = WS_ACC_HLD_AMT;
        CEP.TRC(SCCGWA, DCCURHLD.DATA.RAMT);
        CEP.TRC(SCCGWA, DCCURHLD.DATA.REM_AMT);
        CEP.TRC(SCCGWA, DCCURHLD.DATA.CURR_BAL);
        CEP.TRC(SCCGWA, DCCURHLD.DATA.AVL_AMT);
        CEP.TRC(SCCGWA, DCCURHLD.DATA.ACC_HLD_AMT);
    }
    public void R000_WRITE_DDTHLDR() throws IOException,SQLException,Exception {
        ...
        if (WS_HLD_STS == 'N') {
            ...
            CEP.TRC(SCCGWA, DCCURHLD.DATA.RAMT);
            WS_HLD_AMT = DDRCCY.CURR_BAL - DDRCCY.HOLD_BAL + DDRCCY.CCAL_TOT_BAL - DDRCCY.MARGIN_BAL + DCCURHLD.DATA.RAMT;
        }
        if (DCCURHLD.DATA.DEDUCT_FLG == '1') {
            DDRHLDR.TR_AMT = DCCURHLD.DATA.RAMT;
            CEP.TRC(SCCGWA, WS_HLD_AMT);
            if (WS_HLD_AMT >= 0) {
                DDRHLDR.BEF_TR_AMT = DDRHLD.HLD_AMT - DCCURHLD.DATA.RAMT;
            } else {
        ...
        CEP.TRC(SCCGWA, DDRHLDR.BEF_TR_AMT);
        WS_ACC_HLD_AMT = DDRHLDR.BEF_TR_AMT;
        DDRHLDR.SPR_BR_TYP = WS_SPR_TYP;
        DDRHLDR.CHG_RSN = DCCURHLD.DATA.RSN;
        DDRHLDR.SPR_BR_NM = DCCURHLD.DATA.SPR_NM;
        DDRHLDR.CHG_WRIT_NO = DCCURHLD.DATA.CHG_NO;
        DDRHLDR.LAW_OFF_NM1 = DCCURHLD.DATA.LAW_NM1;
        DDRHLDR.LAW_ID_NO1 = DCCURHLD.DATA.LAW_NO1;
        DDRHLDR.LAW_OFF_NM2 = DCCURHLD.DATA.LAW_NM2;
        DDRHLDR.LAW_ID_NO2 = DCCURHLD.DATA.LAW_NO2;
    ...
    public void R000_UPD_DDTHLDR() throws IOException,SQLException,Exception {
        IBS.init(SCCGWA, DDRHLDR);
        DDRHLDR.KEY.HLD_NO = DCCURHLD.DATA.HLD_NO;
        ...
        if (DCCURHLD.DATA.RAMT == 0 
            && DDRHLDR.TR_AMT != 0) {
            DCCURHLD.DATA.RAMT = DDRHLDR.TR_AMT;
        }
        if (SCCGWA.COMM_AREA.CANCEL_IND == 'Y' 
            && WS_SPR_TYP == '2') {
            if (WS_CANCEL_AVL_AMT < DCCURHLD.DATA.RAMT) {
                WS_ERR_MSG = DDCMSG_ERROR_MSG.DD_AVL_BAL_NOT_ENOUGH;
                S000_ERR_MSG_PROC();
                if (pgmRtn) return;
            }
        }
        DDRHLDR.BEF_TR_AMT = DDRHLD.HLD_AMT + DDRHLDR.TR_AMT;
        CEP.TRC(SCCGWA, DDRHLDR.BEF_TR_AMT);
    ...
    public void R000_UPDATE_DDTHLD() throws IOException,SQLException,Exception {
        ...
        if ((SCCGWA.COMM_AREA.CANCEL_IND != 'Y') 
            || (SCCGWA.COMM_AREA.CANCEL_IND == 'Y' 
            && DCCURHLD.DATA.ABC == 'Y')) {
            DDRHLD.REL_RSN = DCCURHLD.DATA.RSN;
            DDRHLD.REMARK = DCCURHLD.DATA.RMK;
            if (DCCURHLD.DATA.RHLD_TYP == '2') {
                CEP.TRC(SCCGWA, "AAA");
                CEP.TRC(SCCGWA, WS_HLD_AMT);
                CEP.TRC(SCCGWA, DCCURHLD.DATA.RAMT);
                CEP.TRC(SCCGWA, DDRHLD.HLD_AMT);
                DDRHLD.HLD_AMT = DDRHLD.HLD_AMT - DCCURHLD.DATA.RAMT;
        ...
        if (DDRHLD.SPR_BR_TYP == '1' 
            ...
            if ((SCCGWA.COMM_AREA.CANCEL_IND != 'Y') 
                || (SCCGWA.COMM_AREA.CANCEL_IND == 'Y' 
                && DCCURHLD.DATA.ABC == 'Y')) {
                DDRHLD.REL_WRIT_NO = DCCURHLD.DATA.CHG_NO;
            } else {
                DDRHLD.REL_WRIT_NO = " ";
            }
        }
        if ((SCCGWA.COMM_AREA.CANCEL_IND != 'Y') 
            || (SCCGWA.COMM_AREA.CANCEL_IND == 'Y' 
            && DCCURHLD.DATA.ABC == 'Y')) {
            DDRHLD.REL_BR = DCCURHLD.DATA.CHG_BR;
    ...
    public void T000_GROUP_DDTHLDR_RHLD() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, WS_HLDR_RHLD_CNT);
    }
    ...
    public void T000_GROUP_DDTHLD_AC_HLD() throws IOException,SQLException,Exception {
        ...
        DDTHLD_RD.where = "AC = :DDRHLD.AC "
            ...
            + "OR HLD_STS = 'W' )";
        IBS.GROUP(SCCGWA, DDRHLD, this, DDTHLD_RD);
        CEP.TRC(SCCGWA, WS_LAW_AC_HLD_CNT);
        CEP.TRC(SCCGWA, WS_LAW_AC_FBID_CNT);
    }
