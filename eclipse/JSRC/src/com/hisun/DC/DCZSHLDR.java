...
public class DCZSHLDR {
    ...
    char WS_HLDR_FLG = ' ';
    String WS_CARD_NO = " ";
    DCZSHLDR_WS_OUTPUT_HIS_HLD WS_OUTPUT_HIS_HLD = new DCZSHLDR_WS_OUTPUT_HIS_HLD();
    ...
    public void MP(SCCGWA SCCGWA, DCCSHLDR DCCSHLDR) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "DCZSHLDR return!");
        Z_RET();
        if (pgmRtn) return;
        JIBS_RETURN();
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        if (DCCSHLDR.AC.trim().length() == 0 
            && DCCSHLDR.HLD_NO.trim().length() == 0 
            && DCCSHLDR.WRIT_NO.trim().length() == 0) {
            WS_ERR_MSG = DCCMSG_ERROR_MSG.DC_AC_OR_HLDNO_MST_INP;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        if (DCCSHLDR.FO_AC_DATE == 0) {
            DCCSHLDR.FO_AC_DATE = 0;
        }
        if (DCCSHLDR.TO_AC_DATE == 0) {
            DCCSHLDR.TO_AC_DATE = 99991231;
        }
    }
    ...
    public void B050_QUR_HIS_HLD_PROC() throws IOException,SQLException,Exception {
        ...
        if (DCCSHLDR.HLD_NO.trim().length() > 0) {
            DDRHLDR.KEY.HLD_NO = DCCSHLDR.HLD_NO;
            WS_BROWSE_FUNC = 'B';
            R000_BROWSE_DDTHLDR();
            if (pgmRtn) return;
        }
        if (DCCSHLDR.WRIT_NO.trim().length() > 0) {
            WS_BROWSE_FUNC = 'C';
            R000_BROWSE_DDTHLDR();
            if (pgmRtn) return;
        }
    ...
    public void R000_BROWSE_DDTHLDR() throws IOException,SQLException,Exception {
        if (WS_BROWSE_FUNC == 'A') {
            ...
            if (pgmRtn) return;
        } else if (WS_BROWSE_FUNC == 'C') {
            T000_STARTBR_DDTHLD_BY_WRITENO();
            if (pgmRtn) return;
        } else if (WS_BROWSE_FUNC == 'B') {
            T000_STARTBR_DCTHLDR_BY_HLDNO();
            if (pgmRtn) return;
