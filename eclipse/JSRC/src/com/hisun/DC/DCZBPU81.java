...
public class DCZBPU81 {
    ...
    DCRBSP81 DCRBSP81 = new DCRBSP81();
    SCCGWA SCCGWA;
    ...
    public void MP(SCCGWA SCCGWA, SCCRWBSP SCCRWBSP) throws IOException,SQLException,Exception {
        ...
        B00_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "DCZBPU81 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    public void A00_INIT_PROC() throws IOException,SQLException,Exception {
        DCRBSPA = (PSRBSP01) SCCRWBSP.INFO.PTR;
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
        IBS.init(SCCGWA, DCRBSP81);
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DCRBSPA);
        JIBS_tmp_str[1] = JIBS_tmp_str[0];
        IBS.CLONE(SCCGWA, DCRBSPA, DCRBSP81);
    ...
    public void B00_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DCRBSP81);
        JIBS_tmp_str[1] = JIBS_tmp_str[0];
        IBS.CLONE(SCCGWA, DCRBSP81, DCRBSPA);
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DCRBSP81);
    ...
    public void B030_QUERY_RECORD_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, DCRBSP81.KEY.AP_TYPE);
        CEP.TRC(SCCGWA, DCRBSP81.KEY.AP_BATNO);
        CEP.TRC(SCCGWA, DCRBSP81.KEY.BAT_NO);
        T000_READ_BSP();
        if (pgmRtn) return;
        if (SCCRWBSP.RETURN_INFO == 'N') {
            IBS.CPY2CLS(SCCGWA, DCCMSG_ERROR_MSG.DC_UPT_REC_NOTFND, SCCRWBSP.RC);
    ...
    public void T000_READ_BSP() throws IOException,SQLException,Exception {
        DCTBSP81_RD = new DBParm();
        DCTBSP81_RD.TableName = "DCTBSP81";
        IBS.READ(SCCGWA, DCRBSP81, DCTBSP81_RD);
    ...
    public void T000_WRITE_BSP() throws IOException,SQLException,Exception {
        DCTBSP81_RD = new DBParm();
        DCTBSP81_RD.TableName = "DCTBSP81";
        DCTBSP81_RD.errhdl = true;
        IBS.WRITE(SCCGWA, DCRBSP81, DCTBSP81_RD);
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.DBIO_FLG);
        CEP.TRC(SCCGWA, DCRBSP81.BLOB_TR_DATA);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_REWRITE_BSP() throws IOException,SQLException,Exception {
        DCTBSP81_RD = new DBParm();
        DCTBSP81_RD.TableName = "DCTBSP81";
        IBS.REWRITE(SCCGWA, DCRBSP81, DCTBSP81_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
        }
    ...
    public void T000_DELETE_BSP() throws IOException,SQLException,Exception {
        DCTBSP81_RD = new DBParm();
        DCTBSP81_RD.TableName = "DCTBSP81";
        IBS.DELETE(SCCGWA, DCRBSP81, DCTBSP81_RD);
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.DBIO_FLG);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
    ...
    if (SCCGWA.COMM_AREA.BSP_FLG.equalsIgnoreCase("BSP") || SCCGWA.COMM_AREA.CHNL.equalsIgnoreCase("BAT")) { //FROM #IFDEF BAT
        if (SCCRWBSP.RC.RC_CODE != 0) {
            CEP.TRC(SCCGWA, "DCZBPU81: RWBSP-RC =");
