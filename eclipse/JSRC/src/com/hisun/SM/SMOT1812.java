...
public class SMOT1812 {
    boolean pgmRtn = false;
    SMOT1812_WS_TEMP_VARIABLE WS_TEMP_VARIABLE = new SMOT1812_WS_TEMP_VARIABLE();
    short WS_FLD_NO = 0;
    String WS_TABLE_CODE = " ";
    short WS_I = 0;
    short WS_J = 0;
    short WS_CNT = 0;
    String WK_NAME = "        ";
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROCESS();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "SMOT1812 return!");
        Z_RET();
        if (pgmRtn) return;
        JIBS_RETURN();
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        if ((SMB1800_AWA_1800.FLD_CNT > 150) 
            ...
            WS_TEMP_VARIABLE.WS_INFO = " AWA-FLD-CNT IS: " + SMB1800_AWA_1800.FLD_CNT + " BUT MUST NOT LARGER THAN 150 OR " + "EQUAL TO 150 WHEN AWA-FUNC1 = A";
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        if (SMB1800_AWA_1800.FLD_NAME.trim().length() == 0) {
            ...
            WS_TEMP_VARIABLE.WS_INFO = " AWA-FLD-NAME IS: " + SMB1800_AWA_1800.FLD_NAME + " BUT MUST NOT BE SPACE ";
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        ...
        if (WK_NAME_ERROR == 'Y') {
            ...
            WS_FLD_NO = SMB1800_AWA_1800.FLD_NAME_NO;
            WS_TEMP_VARIABLE.WS_INFO = " AWA-FLD-NAME IS: " + SMB1800_AWA_1800.FLD_NAME + " BUT DOES NOT LEAGAL! ";
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        ...
        if (SMB1800_AWA_1800.FLD_DNAM.trim().length() > 0) {
            if (SMB1800_AWA_1800.FLD_CTL == null) SMB1800_AWA_1800.FLD_CTL = "";
            JIBS_tmp_int = SMB1800_AWA_1800.FLD_CTL.length();
            for (int i=0;i<8-JIBS_tmp_int;i++) SMB1800_AWA_1800.FLD_CTL += " ";
            if (!SMB1800_AWA_1800.FLD_CTL.substring(0, 1).equalsIgnoreCase("I") 
                && !SMB1800_AWA_1800.FLD_CTL.substring(0, 1).equalsIgnoreCase("O") 
                && !SMB1800_AWA_1800.FLD_CTL.substring(0, 1).equalsIgnoreCase("G")) {
                IBS.CPY2CLS(SCCGWA, SMCMSG_ERROR_MSG.SM_INPUT_ERROR, WS_TEMP_VARIABLE.WS_MSGID);
                WS_FLD_NO = SMB1800_AWA_1800.FLD_CTL_NO;
                if (SMB1800_AWA_1800.FLD_CTL == null) SMB1800_AWA_1800.FLD_CTL = "";
                JIBS_tmp_int = SMB1800_AWA_1800.FLD_CTL.length();
                for (int i=0;i<8-JIBS_tmp_int;i++) SMB1800_AWA_1800.FLD_CTL += " ";
                WS_TEMP_VARIABLE.WS_INFO = "THE FIRST CONTROL LETTER IS: " + SMB1800_AWA_1800.FLD_CTL + ",BUT MUST BE I/O/G WHEN DIC NAME " + "EXIST!";
                S000_ERR_MSG_PROC();
                if (pgmRtn) return;
            }
        }
    ...
    public void B020_GET_AWADAT() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, SMB1800_AWA_1800.AWA_FMT);
        SCCPRMM.FUNC = '3';
        SCRPRMT.KEY.TYP = "B";
