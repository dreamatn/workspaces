...
public class BPOT9392 {
    int JIBS_tmp_int;
    String JIBS_tmp_str[] = new String[10];
    boolean pgmRtn = false;
    BPOT9392_WS_TEMP_VARIABLE WS_TEMP_VARIABLE = new BPOT9392_WS_TEMP_VARIABLE();
    char WS_CI_RSVD_FLG = ' ';
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROCESS();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "BPOT9392 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B200_BROWSE_CINO_PROC() throws IOException,SQLException,Exception {
        ...
        WS_TEMP_VARIABLE.WS_CINO_CNT = 0;
        CEP.TRC(SCCGWA, WS_TEMP_VARIABLE.WS_CINO_NUM);
        while (WS_TEMP_VARIABLE.WS_CINO_CNT < 50 
            && SCCMPAG.FUNC != 'E') {
            ...
            if (pgmRtn) return;
            CEP.TRC(SCCGWA, WS_CI_RSVD_FLG);
            if (WS_CI_RSVD_FLG == 'N') {
                WS_TEMP_VARIABLE.WS_CINO_CNT = WS_TEMP_VARIABLE.WS_CINO_CNT + 1;
                IBS.init(SCCGWA, SCCMPAG);
                SCCMPAG.FUNC = 'D';
