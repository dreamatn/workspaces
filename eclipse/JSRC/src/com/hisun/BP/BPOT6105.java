...
public class BPOT6105 {
    ...
    short WK_50 = 50;
    short WK_99 = 99;
    BPOT6105_WS_ERR_MSG WS_ERR_MSG = new BPOT6105_WS_ERR_MSG();
    short WS_FLD_NO = 0;
    String WS_CPN = " ";
    short WS_CPN_NO = 0;
    int WS_DATE = 0;
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT6105 return!");
        Z_RET();
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        ...
        BPC6105 = new BPC6105();
        IBS.init(SCCGWA, BPC6105);
        IBS.CPY2CLS(SCCGWA, GWA_SC_AREA.INP_OUTP_AREA.INP_AREA_PTR, BPC6105);
    ...
    public void B100_CHECK_INPUT() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, BPC6105.CLN_FRY);
        WS_CLN_FRY_FLG = BPC6105.CLN_FRY;
        if ((WS_CLN_FRY_FLG != 'D' 
            && WS_CLN_FRY_FLG != 'W' 
            && WS_CLN_FRY_FLG != 'T' 
            && WS_CLN_FRY_FLG != 'M' 
        ...
        CEP.TRC(SCCGWA, BPC6105.CLN_CYC);
        if (BPC6105.CLN_CYC == 0) {
            IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_PAR_INPUT_ERR, WS_ERR_MSG);
            S000_ERR_MSG_PROC_CONTINUE();
        }
        if (WS_CLN_FRY_FLG == 'W') {
            if (BPC6105.CLN_CYC > WK_7) {
                IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_CYC_OVER_SUND, WS_ERR_MSG);
                S000_ERR_MSG_PROC_CONTINUE();
            }
        }
        if (WS_CLN_FRY_FLG == 'T') {
            if (BPC6105.CLN_CYC > WK_10) {
        ...
        if (WS_CLN_FRY_FLG == 'M' 
            || WS_CLN_FRY_FLG == 'Q' 
            || WS_CLN_FRY_FLG == 'H') {
            if (BPC6105.CLN_CYC > WK_31) {
                IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_CYC_OVER_MONT, WS_ERR_MSG);
                S000_ERR_MSG_PROC_CONTINUE();
            }
        }
        if (WS_CLN_FRY_FLG == 'S') {
            WS_DATE = BPC6105.CLN_CYC;
            R000_CHECK_DATE();
        }
        CEP.TRC(SCCGWA, BPC6105.RES_FRY);
        WS_RES_FRY_FLG = BPC6105.RES_FRY;
        if ((WS_RES_FRY_FLG != 'D' 
            && WS_RES_FRY_FLG != 'M' 
            && WS_RES_FRY_FLG != 'Y')) {
            IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_PAR_INPUT_ERR, WS_ERR_MSG);
        ...
        if (WS_RES_FRY_FLG == 'M') {
            if (BPC6105.RES_CYC > WK_99) {
                IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_RES_CYC_OVER_99, WS_ERR_MSG);
                S000_ERR_MSG_PROC_CONTINUE();
            }
            CEP.TRC(SCCGWA, BPC6105.RES_FLG);
            WS_SPLIT_FLG = BPC6105.RES_FLG;
            if ((WS_SPLIT_FLG != 'Y' 
                && WS_SPLIT_FLG != 'N')) {
                IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_PAR_INPUT_ERR, WS_ERR_MSG);
                S000_ERR_MSG_PROC_CONTINUE();
        ...
        CEP.TRC(SCCGWA, BPC6105.RES_CYC);
        if (BPC6105.RES_CYC == 0 
            && WS_RES_FRY_FLG != 'D') {
            IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_PAR_INPUT_ERR, WS_ERR_MSG);
            S000_ERR_MSG_PROC_CONTINUE();
        }
        if (WS_RES_FRY_FLG == 'Y') {
            if (BPC6105.RES_CYC > WK_50) {
                IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_RES_CYC_OVER_50, WS_ERR_MSG);
                S000_ERR_MSG_PROC_CONTINUE();
            }
        }
        CEP.TRC(SCCGWA, BPC6105.STS);
        WS_STS_FLG = BPC6105.STS;
        if ((WS_STS_FLG != 'Y' 
            && WS_STS_FLG != 'N')) {
            IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_PAR_INPUT_ERR, WS_ERR_MSG);
            S000_ERR_MSG_PROC_CONTINUE();
        }
        CEP.TRC(SCCGWA, BPC6105.CLN_FLG);
        WS_CLN_FLG = BPC6105.CLN_FLG;
        if ((WS_CLN_FLG != 'Y' 
            && WS_CLN_FLG != 'N')) {
            IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CLN_PAR_INPUT_ERR, WS_ERR_MSG);
            S000_ERR_MSG_PROC_CONTINUE();
    ...
    public void B200_MAIN_PROCESS() throws IOException,SQLException,Exception {
        IBS.init(SCCGWA, BPCSMCLN);
        BPCSMCLN.BNK = BPC6105.BNK;
        BPCSMCLN.CLN_ID = BPC6105.CLN_ID;
        BPCSMCLN.CLN_RULE = BPC6105.CLN_RULE;
        BPCSMCLN.DES = BPC6105.DES;
        BPCSMCLN.STS = BPC6105.STS;
        BPCSMCLN.CLN_FRY = BPC6105.CLN_FRY;
        BPCSMCLN.CLN_CYC = BPC6105.CLN_CYC;
        BPCSMCLN.RES_FRY = BPC6105.RES_FRY;
        BPCSMCLN.RES_CYC = BPC6105.RES_CYC;
        BPCSMCLN.RES_FLG = BPC6105.RES_FLG;
        BPCSMCLN.CLN_FLG = BPC6105.CLN_FLG;
        BPCSMCLN.CPN = BPC6105.CPN;
        BPCSMCLN.REMARK = BPC6105.REMARK;
    ...
    public void S000_CALL_BPZSMCLN() throws IOException,SQLException,Exception {
        ...
        SCCCALL.ERR_FLDNO = BPC6105.CLN_ID;
