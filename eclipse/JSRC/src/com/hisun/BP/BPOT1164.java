...
public class BPOT1164 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT1164 return!");
        Z_RET();
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        ...
        BPC1164 = new BPC1164();
        IBS.init(SCCGWA, BPC1164);
        IBS.CPY2CLS(SCCGWA, GWA_SC_AREA.INP_OUTP_AREA.INP_AREA_PTR, BPC1164);
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        if (BPC1164.CPNT_ID.trim().length() == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_CPNT_ID_NOTIN;
            S000_ERR_MSG_PROC_CONTINUE();
        }
        IBS.init(SCCGWA, SCCCKDT);
        SCCCKDT.DATE = BPC1164.EFF_DT;
        ...
        if (BPC1164.EFF_DT < SCCGWA.COMM_AREA.AC_DATE) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FEE_EFFDT_LT_ACDT;
            S000_ERR_MSG_PROC_CONTINUE();
        }
        if (BPC1164.EXP_DT <= BPC1164.EFF_DT) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FEE_EFFDT_GT_EXPDT;
            S000_ERR_MSG_PROC_CONTINUE();
        }
        if (BPC1164.DATA[1-1].FEE_CD.trim().length() == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FARM_TYP_ERR;
            S000_ERR_MSG_PROC();
        } else {
            if (BPC1164.DATA[1-1].LNK_FLG == ' ') {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_LNK_FLG_NOTIN;
                S000_ERR_MSG_PROC_CONTINUE();
            } else {
                if (BPC1164.DATA[1-1].LNK_FLG != '0' 
                    && BPC1164.DATA[1-1].LNK_FLG != '1') {
        ...
        for (WS_FEE_NO = 1; WS_FEE_NO <= 20; WS_FEE_NO += 1) {
            if (BPC1164.DATA[WS_FEE_NO-1].FEE_CD.trim().length() > 0) {
                WS_FEE_TEMP = (short) (WS_FEE_NO + 1);
                for (WS_FEE_NEXT = WS_FEE_TEMP; WS_FEE_NEXT <= 20; WS_FEE_NEXT += 1) {
                    if (BPC1164.DATA[WS_FEE_NO-1].FEE_CD.equalsIgnoreCase(BPC1164.DATA[WS_FEE_NEXT-1].FEE_CD)) {
                        WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_AWA_FEE_DUB;
                        S000_ERR_MSG_PROC_CONTINUE();
                    }
                }
                if (BPC1164.DATA[WS_FEE_NO-1].LNK_FLG == ' ') {
                    WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_LNK_FLG_NOTIN;
                    S000_ERR_MSG_PROC_CONTINUE();
                } else {
                    if (BPC1164.DATA[WS_FEE_NO-1].LNK_FLG != '0' 
                        && BPC1164.DATA[WS_FEE_NO-1].LNK_FLG != '1') {
                        WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_LNK_FLG_INVALID;
                        S000_ERR_MSG_PROC_CONTINUE();
                    }
                }
    ...
    public void B020_CREATE_CPN_FEE() throws IOException,SQLException,Exception {
    ...
    public void R000_TRANS_DATA_PARAMETER() throws IOException,SQLException,Exception {
        BPCOFCPN.KEY.CPNT_ID = BPC1164.CPNT_ID;
        BPCOFCPN.VAL.EFF_DATE = BPC1164.EFF_DT;
        BPCOFCPN.VAL.EXP_DATE = BPC1164.EXP_DT;
        for (WS_CNT1 = 1; WS_CNT1 <= 20; WS_CNT1 += 1) {
            BPCOFCPN.VAL.CPN_DATE[WS_CNT1-1].FEE_CODE = BPC1164.DATA[WS_CNT1-1].FEE_CD;
            BPCOFCPN.VAL.CPN_DATE[WS_CNT1-1].LNK_FLG = BPC1164.DATA[WS_CNT1-1].LNK_FLG;
            BPCOFCPN.VAL.CPN_DATE[WS_CNT1-1].STR_CND = BPC1164.DATA[WS_CNT1-1].STR_CND;
        }
