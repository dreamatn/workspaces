...
public class BPOT1063 {
    ...
    SCCGWA SCCGWA;
    BPB1063_AWA_1063 BPB1063_AWA_1063;
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "BPOT1063 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        SCCGWA.COMM_AREA.AWA_AREA_PTR = SCCGWA.COMM_AREA.AWA_AREA_PTR.replaceAll("BODY>", "BPB1063_AWA_1063>");
        BPB1063_AWA_1063 = (BPB1063_AWA_1063) XStreamUtil.xmlToBean(SCCGWA.COMM_AREA.AWA_AREA_PTR);
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
        IBS.init(SCCGWA, BPCSACOB);
    }
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, BPB1063_AWA_1063.AC_FLG);
        CEP.TRC(SCCGWA, BPB1063_AWA_1063.AC_TYPE);
        CEP.TRC(SCCGWA, BPB1063_AWA_1063.ACNO_SEQ);
        B010_CHECK_INPUT();
        if (pgmRtn) return;
        B020_DELETE_ACOBL();
        if (pgmRtn) return;
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        if (BPB1063_AWA_1063.AC_FLG == ' ') {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_AC_TYPE_NOTFND;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        if (BPB1063_AWA_1063.AC_TYPE == ' ') {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_AC_TYPE_NOTFND;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
        if (BPB1063_AWA_1063.ACNO_SEQ == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_E_NO_AC_SEQ;
            S000_ERR_MSG_PROC();
            if (pgmRtn) return;
        }
    }
    public void B020_DELETE_ACOBL() throws IOException,SQLException,Exception {
        IBS.init(SCCGWA, BPCSACOB);
        BPCSACOB.FUNC = 'D';
        B040_TRANS_ACOBL();
        if (pgmRtn) return;
    ...
    public void B040_TRANS_ACOBL() throws IOException,SQLException,Exception {
        BPCSACOB.DATA.AC_FLG = BPB1063_AWA_1063.AC_FLG;
        BPCSACOB.DATA.AC_TYPE = BPB1063_AWA_1063.AC_TYPE;
        BPCSACOB.DATA.AC_NO_SEQ = BPB1063_AWA_1063.ACNO_SEQ;
