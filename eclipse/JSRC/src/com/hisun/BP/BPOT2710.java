...
public class BPOT2710 {
    ...
    SCCGWA SCCGWA;
    BPB2710_AWA_2710 BPB2710_AWA_2710;
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT2710 return!");
        Z_RET();
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        SCCGWA.COMM_AREA.AWA_AREA_PTR = SCCGWA.COMM_AREA.AWA_AREA_PTR.replaceAll("BODY>", "BPB2710_AWA_2710>");
        BPB2710_AWA_2710 = (BPB2710_AWA_2710) XStreamUtil.xmlToBean(SCCGWA.COMM_AREA.AWA_AREA_PTR);
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
    }
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        B010_CHECK_INPUT();
        B020_TURNIN_FALSE_NOTE();
    }
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, BPB2710_AWA_2710.FUNC);
        WS_FUNC_FLG = BPB2710_AWA_2710.FUNC;
        if (WS_FUNC_FLG != 'T') {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FUNC_IS_INVALID;
            WS_FLD_NO = BPB2710_AWA_2710.FUNC_NO;
            S000_ERR_MSG_PROC();
        }
        ...
        IBS.init(SCCGWA, BPCFTLRQ);
        CEP.TRC(SCCGWA, BPB2710_AWA_2710.REC_TLR);
        BPCFTLRQ.INFO.TLR = BPB2710_AWA_2710.REC_TLR;
        S000_CALL_BPZFTLRQ();
        WS_REC_BR = BPCFTLRQ.INFO.TLR_BR;
        CEP.TRC(SCCGWA, WS_REC_BR);
        if (BPCFTLRQ.INFO.TLR_STSW == null) BPCFTLRQ.INFO.TLR_STSW = "";
        JIBS_tmp_int = BPCFTLRQ.INFO.TLR_STSW.length();
        for (int i=0;i<64-JIBS_tmp_int;i++) BPCFTLRQ.INFO.TLR_STSW += " ";
        ...
        if (SCCGWA.COMM_AREA.BR_DP.TR_BRANCH != WS_REC_BR) {
        ...
        BPRTLVB.CRNT_TLR = BPB2710_AWA_2710.REC_TLR;
        ...
        S000_CALL_BPZRTLVB();
        if (WS_REC_PBNO.trim().length() == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_TLVB_NOTFND;
            S000_ERR_MSG_PROC();
        }
        if (BPB2710_AWA_2710.FLS_RECN > K_NUM_20) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_REC_INVALID;
            WS_FLD_NO = BPB2710_AWA_2710.FLS_RECN_NO;
            S000_ERR_MSG_PROC();
        }
        for (WS_CCY = K_NUM_1; WS_CCY <= BPB2710_AWA_2710.FLS_RECN; WS_CCY += K_NUM_1) {
            if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_CCY.trim().length() == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_CCY_IS_SPACE;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_CCY_NO;
                S000_ERR_MSG_PROC();
            } else if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_VAL == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_VAL_IS_SPACE;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_VAL_NO;
                S000_ERR_MSG_PROC();
            } else if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_VER.trim().length() == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_VER_IS_SPACE;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_VER_NO;
                S000_ERR_MSG_PROC();
            } else if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_NUM == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_NUM_IS_ZERO;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_NUM_NO;
                S000_ERR_MSG_PROC();
            } else if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_HDNO.trim().length() == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_HDNO_IS_SPACE;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_HDNO_NO;
                S000_ERR_MSG_PROC();
            } else if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_BGNO.trim().length() == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_BGNO_IS_SPACE;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_BGNO_NO;
                S000_ERR_MSG_PROC();
            } else if (BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_EDNO.trim().length() == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_FLS_EDNO_IS_SPACE;
                WS_FLD_NO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_EDNO_NO;
                S000_ERR_MSG_PROC();
            } else {
            }
        }
    }
    public void B020_TURNIN_FALSE_NOTE() throws IOException,SQLException,Exception {
    ...
    public void R000_TRANS_DATA_PARM() throws IOException,SQLException,Exception {
        BPCOFLSM.FUNC = BPB2710_AWA_2710.FUNC;
        BPCOFLSM.IN_TLR = SCCGWA.COMM_AREA.TL_ID;
        BPCOFLSM.REC_TLR = BPB2710_AWA_2710.REC_TLR;
        BPCOFLSM.TRAN_DT = SCCGWA.COMM_AREA.AC_DATE;
        BPCOFLSM.IN_BR = SCCGWA.COMM_AREA.BR_DP.TR_BRANCH;
        BPCOFLSM.REC_BR = WS_REC_BR;
        BPCOFLSM.PAY_PBNO = WS_PAY_PBNO;
        BPCOFLSM.REC_PBNO = WS_REC_PBNO;
        BPCOFLSM.REC_NUM = BPB2710_AWA_2710.FLS_RECN;
        for (WS_CCY = K_NUM_1; WS_CCY <= BPB2710_AWA_2710.FLS_RECN; WS_CCY += K_NUM_1) {
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_CCY = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_CCY;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_VAL = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_VAL;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_VER = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_VER;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_HDNO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_HDNO;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_BGNO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_BGNO;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_EDNO = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_EDNO;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_NUM = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_NUM;
            BPCOFLSM.CCYS.CCY_DATA[WS_CCY-1].FLS_SRC = BPB2710_AWA_2710.CCYS[WS_CCY-1].FLS_SRC;
        }
    }
    public void S000_CALL_BPZSFLSM() throws IOException,SQLException,Exception {
        ...
        SCCCALL.ERR_FLDNO = BPB2710_AWA_2710.FUNC_NO;
