...
public class BPZOCMWD {
    ...
    public void MP(SCCGWA SCCGWA, BPCPCMWD BPCPCMWD) throws IOException,SQLException,Exception {
        ...
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "BPZOCMWD return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        ...
        for (WS_I = 1; WS_I <= 7; WS_I += 1) {
            if (BPCPCMWD.CAL_CODE[WS_I-1].CNTY_CD.trim().length() == 0 
                && BPCPCMWD.CAL_CODE[WS_I-1].CITY_CD.trim().length() > 0) {
                IBS.CPY2CLS(SCCGWA, BPCMSG_ERROR_MSG.BP_CNTY_CD_M_INPUT, BPCPCMWD.RC);
                Z_RET();
                if (pgmRtn) return;
            }
        ...
        for (WS_I = 1; WS_I <= 7; WS_I += 1) {
            if (BPCPCMWD.CALCD[WS_I-1].trim().length() > 0) {
                ...
                if (BPCPCMWD.CAL_CODE[WS_I-1].CNTY_CD.trim().length() > 0) {
                    ...
                    S000_CALL_BPZQCNCI();
                    if (pgmRtn) return;
                    if (BPCQCNCI.OUTPUT_CITY_DAT.I_CAL_CODE.trim().length() == 0) {
                        WS_CAL_CD[WS_CNT-1] = BPCQCNCI.OUTPUT_CNTY_DAT.CALR_CODE;
                    } else {
                        WS_CAL_CD[WS_CNT-1] = BPCQCNCI.OUTPUT_CITY_DAT.I_CAL_CODE;
                    }
                    WS_CNT += 1;
                }
            }
