...
public class BPOT6112 {
    ...
    BPB6112_AWA_6112 BPB6112_AWA_6112;
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        this.SCCGWA = SCCGWA;
        CEP.TRC(SCCGWA);
        S000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT6112 return!");
        Z_RET();
    }
    public void S000_INIT_PROC() throws IOException,SQLException,Exception {
        SCCGWA.COMM_AREA.AWA_AREA_PTR = SCCGWA.COMM_AREA.AWA_AREA_PTR.replaceAll("BODY>", "BPB6112_AWA_6112>");
        BPB6112_AWA_6112 = (BPB6112_AWA_6112) XStreamUtil.xmlToBean(SCCGWA.COMM_AREA.AWA_AREA_PTR);
    ...
    public void B100_CHECK_PROC() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, BPB6112_AWA_6112.RT_CCY);
        CEP.TRC(SCCGWA, BPB6112_AWA_6112.RT_RBASE);
        CEP.TRC(SCCGWA, BPB6112_AWA_6112.RT_RTENO);
        if (BPB6112_AWA_6112.RT_CCY.trim().length() == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_RATE_MUST_INPUT_1;
            if (BPB6112_AWA_6112.RT_CCY.trim().length() == 0) WS_FLD_NO = 0;
            else WS_FLD_NO = Short.parseShort(BPB6112_AWA_6112.RT_CCY);
            S000_ERR_MSG_NO_PROC();
        }
        if (BPB6112_AWA_6112.RT_RBASE.trim().length() == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_RATE_MUST_INPUT_1;
            if (BPB6112_AWA_6112.RT_RBASE.trim().length() == 0) WS_FLD_NO = 0;
            else WS_FLD_NO = Short.parseShort(BPB6112_AWA_6112.RT_RBASE);
            S000_ERR_MSG_NO_PROC();
        }
        if (BPB6112_AWA_6112.RT_RTENO.trim().length() == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_RATE_MUST_INPUT_1;
            if (BPB6112_AWA_6112.RT_RTENO.trim().length() == 0) WS_FLD_NO = 0;
            else WS_FLD_NO = Short.parseShort(BPB6112_AWA_6112.RT_RTENO);
            S000_ERR_MSG_NO_PROC();
        }
    }
    public void B200_DELETE_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, "DELETE");
        BPCSRTCO.FUNC = 'D';
        BPCSRTCO.INP.INP_CCY = BPB6112_AWA_6112.RT_CCY;
        BPCSRTCO.INP.INP_RBASE = BPB6112_AWA_6112.RT_RBASE;
        BPCSRTCO.INP.INP_RTENO = BPB6112_AWA_6112.RT_RTENO;
        S001_CALL_BPZSRTCO();
        R000_RTN_DATA_PROC();
    }
