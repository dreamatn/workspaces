...
public class BPOT2902 {
    ...
    SCCGWA SCCGWA;
    BPB2902_AWA_2902 BPB2902_AWA_2902;
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT2902 return!");
        Z_RET();
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        SCCGWA.COMM_AREA.AWA_AREA_PTR = SCCGWA.COMM_AREA.AWA_AREA_PTR.replaceAll("BODY>", "BPB2902_AWA_2902>");
        BPB2902_AWA_2902 = (BPB2902_AWA_2902) XStreamUtil.xmlToBean(SCCGWA.COMM_AREA.AWA_AREA_PTR);
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
    }
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, BPB2902_AWA_2902);
        CEP.TRC(SCCGWA, BPB2902_AWA_2902.PLAN_DT);
        CEP.TRC(SCCGWA, BPB2902_AWA_2902.BR);
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, BPB2902_AWA_2902.BR);
        WS_I = 0;
        for (WS_I = 1; WS_I <= K_P_CNT; WS_I += 1) {
            if (BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR.trim().length() > 0) {
                IBS.init(SCCGWA, BPCFTLRQ);
                BPCFTLRQ.INFO.TLR = BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR;
                S000_CALL_BPZFTLRQ();
                if (BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE == '0') {
                    if (BPCFTLRQ.INFO.TLR_STSW == null) BPCFTLRQ.INFO.TLR_STSW = "";
                    JIBS_tmp_int = BPCFTLRQ.INFO.TLR_STSW.length();
                    for (int i=0;i<64-JIBS_tmp_int;i++) BPCFTLRQ.INFO.TLR_STSW += " ";
                    if (!BPCFTLRQ.INFO.TLR_STSW.substring(3 - 1, 3 + 1 - 1).equalsIgnoreCase("1")) {
                        CEP.ERR(SCCGWA, BPCMSG_ERROR_MSG.BP_INPUT_ERROR83, BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR_NO);
                    }
                }
                if (BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE == '1') {
                    if (BPCFTLRQ.INFO.TLR_STSW == null) BPCFTLRQ.INFO.TLR_STSW = "";
                    JIBS_tmp_int = BPCFTLRQ.INFO.TLR_STSW.length();
                    for (int i=0;i<64-JIBS_tmp_int;i++) BPCFTLRQ.INFO.TLR_STSW += " ";
                    if (!BPCFTLRQ.INFO.TLR_STSW.substring(0, 1).equalsIgnoreCase("1")) {
                        CEP.ERR(SCCGWA, BPCMSG_ERROR_MSG.BP_INPUT_ERROR84, BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR_NO);
                    }
                }
            }
        }
        WS_I = 0;
        for (WS_I = 1; WS_I <= K_P_CNT; WS_I += 1) {
            CEP.TRC(SCCGWA, BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE);
            if (BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE == '1') {
                IBS.init(SCCGWA, BPRVHPB);
                ...
                if (BPCRVHPB.RETURN_INFO == 'N' 
                    && BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR.trim().length() > 0) {
                    CEP.ERR(SCCGWA, BPCMSG_ERROR_MSG.BP_NOT_HAVE_BVBOX, BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR_NO);
                }
            }
            if (BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE == '0') {
                IBS.init(SCCGWA, BPRTLVB);
        ...
        if (BPB2902_AWA_2902.PLAN_DT == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_INPUT_DATE_ERROR;
            S000_ERR_MSG_PROC();
        }
        if (BPB2902_AWA_2902.BR == 0) {
            WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_BRANCE_MUST_INPUT;
            S000_ERR_MSG_PROC();
        }
        IBS.init(SCCGWA, BPRBOXP);
        BPRBOXP.KEY.PLAN_DATE = BPB2902_AWA_2902.PLAN_DT;
        BPRBOXP.KEY.BR = BPB2902_AWA_2902.BR;
        ...
        S000_CALL_BPZRBOXP();
        WS_I = 0;
        for (WS_I = 2; WS_I <= K_P_CNT 
            && BPCSBOXM.PLAN_INFO[WS_I-1].BOX_TYPE != ' '; WS_I += 1) {
            for (WS_J = 1; WS_I != WS_J 
                && BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE != ' '; WS_J += 1) {
                if (BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE == BPB2902_AWA_2902.PLAN_INF[WS_J-1].BOX_TYPE 
                    && BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR.equalsIgnoreCase(BPB2902_AWA_2902.PLAN_INF[WS_J-1].PLAN_TLR) 
                    && BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR.trim().length() > 0) {
                    CEP.TRC(SCCGWA, WS_I);
                    CEP.TRC(SCCGWA, WS_J);
    ...
    public void R000_TRANS_DATA_PARAMETER() throws IOException,SQLException,Exception {
        BPCSBOXM.PLAN_DT = BPB2902_AWA_2902.PLAN_DT;
        BPCSBOXM.BR = BPB2902_AWA_2902.BR;
        for (WS_I = 1; WS_I <= K_P_CNT; WS_I += 1) {
            BPCSBOXM.PLAN_INFO[WS_I-1].BOX_TYPE = BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_TYPE;
            BPCSBOXM.PLAN_INFO[WS_I-1].BOX_NO = BPB2902_AWA_2902.PLAN_INF[WS_I-1].BOX_NO;
            BPCSBOXM.PLAN_INFO[WS_I-1].CRNT_TLR = BPB2902_AWA_2902.PLAN_INF[WS_I-1].CRNT_TLR;
            BPCSBOXM.PLAN_INFO[WS_I-1].PLAN_TLR = BPB2902_AWA_2902.PLAN_INF[WS_I-1].PLAN_TLR;
        }
    }
