...
public class BPOT5084 {
    ...
    BPB5084_AWA_5084 BPB5084_AWA_5084;
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        this.SCCGWA = SCCGWA;
        CEP.TRC(SCCGWA);
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT5084 return!");
        Z_RET();
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        SCCGWA.COMM_AREA.AWA_AREA_PTR = SCCGWA.COMM_AREA.AWA_AREA_PTR.replaceAll("BODY>", "BPB5084_AWA_5084>");
        BPB5084_AWA_5084 = (BPB5084_AWA_5084) XStreamUtil.xmlToBean(SCCGWA.COMM_AREA.AWA_AREA_PTR);
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
    }
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        IBS.init(SCCGWA, BPCEXRP);
        if (BPB5084_AWA_5084.DATA[1-1].EXR_TYP.trim().length() == 0) {
            CEP.ERR(SCCGWA, FXCMSG_ERROR_MSG.FX_EX_TYP_ERR);
        }
        for (WS_I = 1; BPB5084_AWA_5084.DATA[WS_I-1].EXR_TYP.trim().length() != 0; WS_I += 1) {
            BPCEXRP.EXR_TYP = BPB5084_AWA_5084.DATA[WS_I-1].EXR_TYP;
            BPCEXRP.BR = BPB5084_AWA_5084.DATA[WS_I-1].BR;
            BPCEXRP.CCY = BPB5084_AWA_5084.DATA[WS_I-1].CCY;
            BPCEXRP.CMP_FLG = BPB5084_AWA_5084.DATA[WS_I-1].CMP_FLG;
            BPCEXRP.HCS_BP = BPB5084_AWA_5084.DATA[WS_I-1].HCS_BP;
            BPCEXRP.HFX_BP = BPB5084_AWA_5084.DATA[WS_I-1].HFX_BP;
            BPCEXRP.HCS_SP = BPB5084_AWA_5084.DATA[WS_I-1].HCS_SP;
            BPCEXRP.HFX_SP = BPB5084_AWA_5084.DATA[WS_I-1].HFX_SP;
            B020_CHECK_INPUT();
            B020_ADD_REC_PROC();
        }
    }
