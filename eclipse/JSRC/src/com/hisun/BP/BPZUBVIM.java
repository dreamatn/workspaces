...
public class BPZUBVIM {
    ...
    int WS_CNT = 0;
    int WS_POS = 0;
    BPZUBVIM_WS_EWA_AC_NO WS_EWA_AC_NO = new BPZUBVIM_WS_EWA_AC_NO();
    ...
    public void MP(SCCGWA SCCGWA, BPCUBVIM BPCUBVIM) throws IOException,SQLException,Exception {
        this.SCCGWA = SCCGWA;
        this.BPCUBVIM = BPCUBVIM;
        CEP.TRC(SCCGWA);
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPZUBVIM return!");
        Z_RET();
    }
    ...
    public void B010_CHK_INPUT_CH() throws IOException,SQLException,Exception {
        if (BPCUBVIM.BV_CODE.trim().length() == 0) {
    ...
    public void B020_CHK_BV_PARM() throws IOException,SQLException,Exception {
        ...
        BPCFBVQU.TX_DATA.KEY.CODE = BPCUBVIM.BV_CODE;
        S000_CALL_BPZFBVQU();
        if (SCCGWA.COMM_AREA.CANCEL_IND == 'Y' 
            && (BPCFBVQU.TX_DATA.CTL_FLG != '0')) {
            IBS.init(SCCGWA, BPRBUSE);
            IBS.init(SCCGWA, BPCRBUSE);
            BPRBUSE.KEY.BR = BPCFTLRQ.INFO.TLR_BR;
            BPRBUSE.KEY.BV_CODE = BPCUBVIM.BV_CODE;
            BPRBUSE.KEY.HEAD_NO = BPCUBVIM.HEAD_NO;
            BPRBUSE.KEY.BEG_NO = BPCUBVIM.BEG_NO;
            BPRBUSE.KEY.END_NO = BPCUBVIM.END_NO;
            ...
            if (BPRBUSE.TX_BR != BPCFTLRQ.INFO.TLR_BR) {
        ...
        if (BPCUBVIM.HEAD_NO.trim().length() > 0) {
            CEP.TRC(SCCGWA, "CHK HEAD NO!");
            CEP.TRC(SCCGWA, BPCUBVIM.HEAD_NO);
            if (BPCUBVIM.HEAD_NO == null) BPCUBVIM.HEAD_NO = "";
            JIBS_tmp_int = BPCUBVIM.HEAD_NO.length();
            for (int i=0;i<10-JIBS_tmp_int;i++) BPCUBVIM.HEAD_NO += " ";
            for (WS_I = 1; WS_I <= 10 
                && BPCUBVIM.HEAD_NO.substring(WS_I - 1, WS_I + 1 - 1).trim().length() != 0; WS_I += 1) {
            }
        ...
        if (BPCUBVIM.BEG_NO.trim().length() > 0) {
            CEP.TRC(SCCGWA, "CHK BEG NO!");
            if (BPCUBVIM.BEG_NO == null) BPCUBVIM.BEG_NO = "";
            JIBS_tmp_int = BPCUBVIM.BEG_NO.length();
            for (int i=0;i<20-JIBS_tmp_int;i++) BPCUBVIM.BEG_NO += " ";
            for (WS_I = 1; WS_I <= 20 
                && IBS.isNumeric(BPCUBVIM.BEG_NO.substring(WS_I - 1, WS_I + 1 - 1)); WS_I += 1) {
            }
        ...
        if (BPCUBVIM.END_NO.trim().length() > 0) {
            CEP.TRC(SCCGWA, "CHK END NO!");
            if (BPCUBVIM.END_NO == null) BPCUBVIM.END_NO = "";
            JIBS_tmp_int = BPCUBVIM.END_NO.length();
            for (int i=0;i<20-JIBS_tmp_int;i++) BPCUBVIM.END_NO += " ";
            for (WS_I = 1; WS_I <= 20 
                && IBS.isNumeric(BPCUBVIM.END_NO.substring(WS_I - 1, WS_I + 1 - 1)); WS_I += 1) {
            }
            if (BPCFBVQU.TX_DATA.NO_LENGTH != WS_I - 1) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_ERR_BVNO_LEN;
                S000_ERR_MSG_PROC();
        ...
        if (BPCFBVQU.TX_DATA.CTL_FLG == '1' 
            ...
            JIBS_tmp_int = BPCUBVIM.BEG_NO.length();
            for (int i=0;i<20-JIBS_tmp_int;i++) BPCUBVIM.BEG_NO += " ";
            if (BPCUBVIM.BEG_NO.substring(0, WS_BVNO_LEN).trim().length() == 0) WS_COMP_BEGNO = 0;
            else WS_COMP_BEGNO = Long.parseLong(BPCUBVIM.BEG_NO.substring(0, WS_BVNO_LEN));
            if (BPCUBVIM.END_NO == null) BPCUBVIM.END_NO = "";
            JIBS_tmp_int = BPCUBVIM.END_NO.length();
            for (int i=0;i<20-JIBS_tmp_int;i++) BPCUBVIM.END_NO += " ";
            if (BPCUBVIM.END_NO.substring(0, WS_BVNO_LEN).trim().length() == 0) WS_COMP_ENDNO = 0;
            else WS_COMP_ENDNO = Long.parseLong(BPCUBVIM.END_NO.substring(0, WS_BVNO_LEN));
            if (WS_COMP_BEGNO > WS_COMP_ENDNO) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_BEG_END;
                S000_ERR_MSG_PROC();
            }
            IBS.init(SCCGWA, BPCSNOCK);
            BPCSNOCK.BV_CODE = BPCUBVIM.BV_CODE;
            BPCSNOCK.BEG_NO = BPCUBVIM.BEG_NO;
            BPCSNOCK.END_NO = BPCUBVIM.END_NO;
            BPCSNOCK.NUM = BPCUBVIM.NUM;
            BPCSNOCK.FUNC = '1';
            S000_CALL_BPZSNOCK();
        }
        if (BPCFBVQU.TX_DATA.TYPE == '1') {
            CEP.TRC(SCCGWA, "CHK BL INF!");
            if (BPCUBVIM.CCY.trim().length() == 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_CCY_MUST_INPUT;
                S000_ERR_MSG_PROC();
            } else {
                if (!BPCUBVIM.CCY.equalsIgnoreCase(BPCFBVQU.TX_DATA.CCY)) {
            ...
            CEP.TRC(SCCGWA, BPCUBVIM.PVAL);
            CEP.TRC(SCCGWA, BPCUBVIM.NUM);
            CEP.TRC(SCCGWA, BPCUBVIM.AMT);
            if (BPCUBVIM.AMT != BPCUBVIM.PVAL * BPCUBVIM.NUM) {
    ...
    public void B030_CHK_BV_TLR_CH() throws IOException,SQLException,Exception {
        ...
        BPCFTLRQ.INFO.TLR = BPCUBVIM.TLR;
        S000_CALL_BPZFTLRQ();
        CEP.TRC(SCCGWA, BPCFTLRQ.INFO.TLR_BR);
        IBS.init(SCCGWA, BPCPCLIB);
        BPCPCLIB.TLR = BPCUBVIM.TLR;
        BPCPCLIB.VB_FLG = BPCUBVIM.VB_FLG;
        if (BPCFBVQU.TX_DATA.TYPE != '1') {
            IBS.init(SCCGWA, BPRVHPB);
            IBS.init(SCCGWA, BPCRVHPB);
            BPRVHPB.BR = BPCFTLRQ.INFO.TLR_BR;
            BPRVHPB.CUR_TLR = BPCUBVIM.TLR;
            BPRVHPB.POLL_BOX_IND = BPCUBVIM.VB_FLG;
            ...
            if (BPCRVHPB.RETURN_INFO == 'F') {
                ...
                if (BPCUBVIM.PLBOX.trim().length() == 0) {
                    BPCUBVIM.PLBOX = BPRVHPB.KEY.POOL_BOX_NO;
                }
    ...
    public void B050_PROCESS_MOV_IN_CH() throws IOException,SQLException,Exception {
        ...
        BPCCBVIO.BR = BPCFTLRQ.INFO.TLR_BR;
        BPCCBVIO.TLR = BPCUBVIM.TLR;
        BPCCBVIO.PLBOX_NO = WS_TEMP_PLBOX_NO;
        BPCCBVIO.VB_FLG = BPCUBVIM.VB_FLG;
        BPCCBVIO.BV_CODE = BPCUBVIM.BV_CODE;
        BPCCBVIO.CTL_FLG = BPCFBVQU.TX_DATA.CTL_FLG;
        BPCCBVIO.PVAL = BPCUBVIM.PVAL;
        BPCCBVIO.TYPE = BPCUBVIM.TYPE;
        BPCCBVIO.HEAD_NO = BPCUBVIM.HEAD_NO;
        BPCCBVIO.BEG_NO = BPCUBVIM.BEG_NO;
        BPCCBVIO.END_NO = BPCUBVIM.END_NO;
        BPCCBVIO.NUM = BPCUBVIM.NUM;
        BPCCBVIO.BV_STS = BPCUBVIM.BV_STS;
        BPCCBVIO.UPD_TLR = BPRTLT.KEY.TLR;
        BPCCBVIO.BVNO_LEN = WS_BVNO_LEN;
        CEP.TRC(SCCGWA, BPCUBVIM.VIL_TYP);
        BPCCBVIO.VIL_TYP = BPCUBVIM.VIL_TYP;
    ...
    public void B060_01_ADD_BUSE_PROCESS() throws IOException,SQLException,Exception {
        ...
        BPRBUSE.TX_BR = BPCFTLRQ.INFO.TLR_BR;
    ...
    public void R_CHK_BV_NUM() throws IOException,SQLException,Exception {
        if (BPCCBVIO.CTL_FLG == '0') {
            ...
            BPRTBVD.KEY.PL_BOX_NO = BPCUBVIM.PLBOX;
            BPRTBVD.KEY.BV_CODE = BPCUBVIM.BV_CODE;
            BPRTBVD.KEY.VALUE = BPCUBVIM.PVAL;
            ...
            BPRTBVD.KEY.BV_CODE = BPCUBVIM.BV_CODE;
            BPRTBVD.KEY.VALUE = BPCUBVIM.PVAL;
            BPRTBVD.KEY.HEAD_NO = BPCUBVIM.HEAD_NO;
            BPRTBVD.KEY.STS = K_STS_NORMAL;
    ...
    public void R_CHK_BV_LEAST_NO() throws IOException,SQLException,Exception {
        ...
        BPRTBVD.KEY.VALUE = BPCUBVIM.PVAL;
        BPRTBVD.KEY.HEAD_NO = BPCUBVIM.HEAD_NO;
        BPRTBVD.BEG_NO = BPCUBVIM.BEG_NO;
        BPRTBVD.KEY.END_NO = BPCUBVIM.END_NO;
        BPRTBVD.KEY.STS = BPCUBVIM.BV_STS;
        IBS.init(SCCGWA, BPCRTBDB);
        ...
        if (!BPRTBVD.BEG_NO.equalsIgnoreCase(BPCUBVIM.BEG_NO)) {
