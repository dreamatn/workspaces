...
public class BPOT4841 {
    ...
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        ...
        A000_INIT_PROC();
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "BPOT4841 return!");
        Z_RET();
    }
    ...
    public void B010_CHECK_INPUT() throws IOException,SQLException,Exception {
        WS_BR_LEN = 6;
        CEP.TRC(SCCGWA, WS_BR_LEN);
        WS_BR_LEN += 1;
        if (BPB4840_AWA_4840.ASS_TYP == 'O') {
            if (BPB4840_AWA_4840.ASS_ID == null) BPB4840_AWA_4840.ASS_ID = "";
            JIBS_tmp_int = BPB4840_AWA_4840.ASS_ID.length();
            for (int i=0;i<30-JIBS_tmp_int;i++) BPB4840_AWA_4840.ASS_ID += " ";
            if (BPB4840_AWA_4840.ASS_ID.substring(WS_BR_LEN - 1, WS_BR_LEN + 1 - 1).trim().length() > 0) {
                WS_ERR_MSG = BPCMSG_ERROR_MSG.BP_INPUT_ERROR;
                WS_FLD_NO = BPB4840_AWA_4840.ASS_ID_NO;
                S000_ERR_MSG_PROC();
            }
        }
    }
    public void B020_ADD_REC_PROC() throws IOException,SQLException,Exception {
        ...
        CEP.TRC(SCCGWA, BPCSMGRL.FUNC);
        CEP.TRC(SCCGWA, BPCSMGRL.MODIFY_FUNC);
        S00_CALL_BPZSMGRL();
    }
