...
public class BAZUGD01 {
    ...
    SCCBATH SCCBATH;
    BACUGD01 BACUGD01;
    public void MP(SCCGWA SCCGWA, BACUGD01 BACUGD01) throws IOException,SQLException,Exception {
        this.SCCGWA = SCCGWA;
        this.BACUGD01 = BACUGD01;
        CEP.TRC(SCCGWA);
        A00_INIT_PROC();
        if (pgmRtn) return;
        B00_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "BAZUGD01 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    public void A00_INIT_PROC() throws IOException,SQLException,Exception {
        SCCBATH = (SCCBATH) SCCGWA.COMM_AREA.BAT_AREA_PTR;
        BACUGD01.RC.RC_MMO = "LN";
        BACUGD01.RC.RC_CODE = 0;
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.JRN_NO);
        CEP.TRC(SCCGWA, SCCBATH.JPRM.AC_DATE);
        CEP.TRC(SCCGWA, SCCBATH.JPRM.NEXT_AC_DATE);
        CEP.TRC(SCCGWA, BACUGD01.DATA.BILL_NO);
    }
    public void B00_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        if (pgmRtn) return;
        for (WS_GUAR_NUM = 1; WS_GUAR_NUM <= GDCIQLDR.TCNT 
            && WS_TMP_AMT > 0; WS_GUAR_NUM += 1) {
            B300_DEDUC_FROM_DEPOSIT();
            if (pgmRtn) return;
        }
        B200_INQUIRE_DEPOSIT_INF();
        if (pgmRtn) return;
    ...
    public void B000_GET_MST1_INF() throws IOException,SQLException,Exception {
        ...
        BACFMST1.FUNC = 'T';
        CEP.TRC(SCCGWA, BACUGD01.DATA.BILL_NO);
        BARMST1.BILL_NO = BACUGD01.DATA.BILL_NO;
        S000_CALL_BAZFMST1();
        if (pgmRtn) return;
    ...
    public void B300_DEDUC_FROM_DEPOSIT() throws IOException,SQLException,Exception {
        IBS.init(SCCGWA, GDCUMPDR);
        GDCUMPDR.INPUT.FUNC = 'D';
        GDCUMPDR.INPUT.RSEQ = BARMST1.B_REF_NO;
        CEP.TRC(SCCGWA, GDCUMPDR.INPUT.RSEQ);
        CEP.TRC(SCCGWA, GDCIQLDR.INFO[WS_GUAR_NUM-1].OUT_AC);
        CEP.TRC(SCCGWA, GDCIQLDR.INFO[WS_GUAR_NUM-1].OUT_SEQ);
        GDCUMPDR.INPUT.AC = GDCIQLDR.INFO[WS_GUAR_NUM-1].OUT_AC;
        GDCUMPDR.INPUT.SEQ = GDCIQLDR.INFO[WS_GUAR_NUM-1].OUT_SEQ;
        CEP.TRC(SCCGWA, WS_TMP_AMT);
        CEP.TRC(SCCGWA, GDCIQLDR.INFO[WS_GUAR_NUM-1].RAMT);
        if (WS_TMP_AMT > GDCIQLDR.INFO[WS_GUAR_NUM-1].RAMT 
            && GDCIQLDR.INFO[WS_GUAR_NUM-1].RAMT != 0) {
            GDCUMPDR.INPUT.AMT = GDCIQLDR.INFO[WS_GUAR_NUM-1].RAMT;
        } else {
            GDCUMPDR.INPUT.AMT = WS_TMP_AMT;
        }
        S000_CALL_GDZUMPDR();
        if (pgmRtn) return;
        WS_GUAR_TX_AMT += GDCUMPDR.INPUT.AMT;
        WS_TMP_AMT = WS_TMP_AMT - GDCUMPDR.INPUT.AMT;
        CEP.TRC(SCCGWA, WS_GUAR_TX_AMT);
        CEP.TRC(SCCGWA, WS_TMP_AMT);
    }
    ...
    public void B600_SET_EWA_EVENT() throws IOException,SQLException,Exception {
        ...
        BPCPOEWA.DATA.AMT_INFO[12-1].AMT = WS_GUAR_TX_AMT;
        ...
        if (JIBS_tmp_str[0].equalsIgnoreCase("662170")) {
            ...
            if (BARMST1.BILL_TYP == 'E') {
                BPCPOEWA.DATA.AMT_INFO[15-1].AMT = WS_GUAR_TX_AMT;
            } else {
                B700_REM_CR_PROCESS();
                if (pgmRtn) return;
            }
            ...
            BPCPOEWA.DATA.AMT_INFO[22-1].AMT = WS_GUAR_TX_AMT;
    ...
    public void B700_REM_CR_PROCESS() throws IOException,SQLException,Exception {
        ...
        AICUUPIA.DATA.AMT = WS_GUAR_TX_AMT;
    ...
    public void B900_CREATE_TXDL_PROC() throws IOException,SQLException,Exception {
        ...
        BARTXDL.FUN_CD = BACUGD01.DATA.FUN_CD;
    ...
    public void S000_CALL_BAZFTXDL() throws IOException,SQLException,Exception {
        ...
        if (BACFTXDL.RC.RC_CODE != 0) {
            ...
            IBS.CPY2CLS(SCCGWA, JIBS_tmp_str[0], BACUGD01.RC);
            Z_RET();
            if (pgmRtn) return;
        }
    }
