...
public class PSOT1210 {
    DBParm PSTEINF_RD;
    String K_EXG_BK_NO = "001";
    String K_OUTPUT_FMT = "PS121";
    String WS_ERR_MSG = " ";
    String WS_ERR_INFO = " ";
    int WS_ISS_BR_MSG = 0;
    char WS_TABLE_FLG = ' ';
    PSOT1210_WS_FMT WS_FMT = new PSOT1210_WS_FMT();
    short WS_TST_YES = 0;
    short WS_TST_MTH = 0;
    short WS_TST_DAY = 0;
    short WS_TST_NUM = 0;
    ...
    PSB1210_AWA_1210 PSB1210_AWA_1210;
    public void MP(SCCGWA SCCGWA) throws IOException,SQLException,Exception {
        this.SCCGWA = SCCGWA;
        CEP.TRC(SCCGWA);
        A000_INIT_PROC();
        CEP.TRC(SCCGWA, PSB1210_AWA_1210.EXG_DT);
        B000_MAIN_PROC();
        CEP.TRC(SCCGWA, "PSOT1210 return!");
        Z_RET();
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
        SCCGWA.COMM_AREA.AWA_AREA_PTR = SCCGWA.COMM_AREA.AWA_AREA_PTR.replaceAll("BODY>", "PSB1210_AWA_1210>");
        PSB1210_AWA_1210 = (PSB1210_AWA_1210) XStreamUtil.xmlToBean(SCCGWA.COMM_AREA.AWA_AREA_PTR);
    }
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, PSB1210_AWA_1210.EXG_DT);
        if (PSB1210_AWA_1210.EXG_DT == 0) {
            CEP.TRC(SCCGWA, 1111);
            if (PSB1210_AWA_1210.EXG_TMS == 0) {
                CEP.TRC(SCCGWA, 0);
                B100_GET_DATE_TMS();
            }
        } else {
    ...
    public void B200_CHEK_DATE() throws IOException,SQLException,Exception {
        if (PSB1210_AWA_1210.EXG_DT < SCCGWA.COMM_AREA.AC_DATE) {
            WS_ERR_MSG = PSCMSG_ERROR_MSG.PS_STDT_MST_INPUT;
            S000_ERR_MSG_PROC();
        }
        if (PSB1210_AWA_1210.EXG_DT == SCCGWA.COMM_AREA.AC_DATE) {
            if (PSB1210_AWA_1210.EXG_TMS > PSREINF.EXG_JOIN_TMS) {
