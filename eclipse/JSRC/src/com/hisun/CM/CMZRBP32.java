...
public class CMZRBP32 {
    DBParm CMTBSP32_RD;
    brParm CMTBSP32_BR = new brParm();
    boolean pgmRtn = false;
    String WS_BSPNAME = " ";
    char WS_TBL_FARM_FLAG = ' ';
    CMCMSG_ERROR_MSG CMCMSG_ERROR_MSG = new CMCMSG_ERROR_MSG();
    SCCEXCP SCCEXCP = new SCCEXCP();
    CMRBSP32 CMRBSP32 = new CMRBSP32();
    SCCGWA SCCGWA;
    ...
    public void MP(SCCGWA SCCGWA, SCCRWBSP SCCRWBSP) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "CMZRBP32 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        CMRLSP32 = (PSRBSP01) SCCRWBSP.INFO.PTR;
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
        IBS.init(SCCGWA, CMRBSP32);
        IBS.CLONE(SCCGWA, CMRLSP32, CMRBSP32);
    ...
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        IBS.CLONE(SCCGWA, CMRBSP32, CMRLSP32);
    }
    ...
    public void B020_READUPD_RECORD_PROC() throws IOException,SQLException,Exception {
        ...
        if (SCCRWBSP.RETURN_INFO == 'N') {
            IBS.CPY2CLS(SCCGWA, CMCMSG_ERROR_MSG.CM_BSP32_REC_NOT_EXIST, SCCRWBSP.RC);
            Z_RET();
            if (pgmRtn) return;
        }
    }
    ...
    public void T000_READ_BSP() throws IOException,SQLException,Exception {
        CMTBSP32_RD = new DBParm();
        CMTBSP32_RD.TableName = "CMTBSP32";
        IBS.READ(SCCGWA, CMRBSP32, CMTBSP32_RD);
    ...
    public void T000_WRITE_BSP() throws IOException,SQLException,Exception {
        CMTBSP32_RD = new DBParm();
        CMTBSP32_RD.TableName = "CMTBSP32";
        CMTBSP32_RD.errhdl = true;
        IBS.WRITE(SCCGWA, CMRBSP32, CMTBSP32_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
            ...
            IBS.init(SCCGWA, SCCEXCP);
            SCCEXCP.MSG_TEXT.MSG_TEXT_D.TABLE_NAME = "CMTBSP32";
    ...
    public void T000_READ_BSP_UPD() throws IOException,SQLException,Exception {
        CMTBSP32_RD = new DBParm();
        CMTBSP32_RD.TableName = "CMTBSP32";
        CMTBSP32_RD.upd = true;
        CMTBSP32_RD.col = "STATUS, RT_CODE, OUT_JRN, OUT_VCH_NO, OUT_DATA, TS";
        IBS.READ(SCCGWA, CMRBSP32, CMTBSP32_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
    ...
    public void T000_REWRITE_BSP() throws IOException,SQLException,Exception {
        CMTBSP32_RD = new DBParm();
        CMTBSP32_RD.TableName = "CMTBSP32";
        IBS.REWRITE(SCCGWA, CMRBSP32, CMTBSP32_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
        }
    ...
    public void T000_DELETE_BSP() throws IOException,SQLException,Exception {
        CMTBSP32_RD = new DBParm();
        CMTBSP32_RD.TableName = "CMTBSP32";
        IBS.DELETE(SCCGWA, CMRBSP32, CMTBSP32_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
            SCCRWBSP.RETURN_INFO = 'N';
    ...
    public void T000_STARTBR_BSP() throws IOException,SQLException,Exception {
        CMTBSP32_BR.rp = new DBParm();
        CMTBSP32_BR.rp.TableName = "CMTBSP32";
        CMTBSP32_BR.rp.upd = true;
        IBS.STARTBR(SCCGWA, CMRBSP32, CMTBSP32_BR);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
        }
    ...
    public void T000_READNEXT_BSP() throws IOException,SQLException,Exception {
        IBS.READNEXT(SCCGWA, CMRBSP32, this, CMTBSP32_BR);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
            SCCRWBSP.RETURN_INFO = 'N';
