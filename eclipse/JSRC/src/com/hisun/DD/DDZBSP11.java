...
public class DDZBSP11 {
    ...
    DDRBSP11 DDRBSP11 = new DDRBSP11();
    SCCGWA SCCGWA;
    ...
    public void MP(SCCGWA SCCGWA, SCCRWBSP SCCRWBSP) throws IOException,SQLException,Exception {
        ...
        B000_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "DDZBSP11 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    public void A000_INIT_PROC() throws IOException,SQLException,Exception {
        DDRBSPLK = (PSRBSP01) SCCRWBSP.INFO.PTR;
        GWA_BP_AREA = (SCCGBPA_BP_AREA) SCCGWA.BP_AREA_PTR;
        GWA_SC_AREA = (SCCGSCA_SC_AREA) SCCGWA.SC_AREA_PTR;
        IBS.init(SCCGWA, DDRBSP11);
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DDRBSPLK);
        JIBS_tmp_str[1] = JIBS_tmp_str[0];
        IBS.CLONE(SCCGWA, DDRBSPLK, DDRBSP11);
    ...
    public void B000_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DDRBSP11);
        JIBS_tmp_str[1] = JIBS_tmp_str[0];
        IBS.CLONE(SCCGWA, DDRBSP11, DDRBSPLK);
    }
    public void B010_CREATE_RECORD_PROC() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, DDRBSP11);
        T000_WRITE_BSP();
        if (pgmRtn) return;
        if (SCCRWBSP.RETURN_INFO == 'D') {
            IBS.CPY2CLS(SCCGWA, DDCMSG_ERROR_MSG.DD_IS_DUPKEY, SCCRWBSP.RC);
            Z_RET();
            if (pgmRtn) return;
        }
    }
    ...
    public void T000_READ_BSP() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, DDRBSP11.KEY.AP_TYPE);
        DDTBSP11_RD = new DBParm();
        DDTBSP11_RD.TableName = "DDTBSP11";
        IBS.READ(SCCGWA, DDRBSP11, DDTBSP11_RD);
    ...
    public void T000_WRITE_BSP() throws IOException,SQLException,Exception {
        DDTBSP11_RD = new DBParm();
        DDTBSP11_RD.TableName = "DDTBSP11";
        DDTBSP11_RD.errhdl = true;
        IBS.WRITE(SCCGWA, DDRBSP11, DDTBSP11_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_READ_BSP_UPD() throws IOException,SQLException,Exception {
        DDTBSP11_RD = new DBParm();
        DDTBSP11_RD.TableName = "DDTBSP11";
        DDTBSP11_RD.upd = true;
        DDTBSP11_RD.col = "STATUS, RT_CODE, OUT_JRN, OUT_VCH_NO, OUT_DATA, TS";
        IBS.READ(SCCGWA, DDRBSP11, DDTBSP11_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
    ...
    public void T000_REWRITE_BSP() throws IOException,SQLException,Exception {
        DDTBSP11_RD = new DBParm();
        DDTBSP11_RD.TableName = "DDTBSP11";
        IBS.REWRITE(SCCGWA, DDRBSP11, DDTBSP11_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
            IBS.CPY2CLS(SCCGWA, DDCMSG_ERROR_MSG.DD_SYS_ERR, SCCRWBSP.RC);
    ...
    public void T000_DELETE_BSP() throws IOException,SQLException,Exception {
        DDTBSP11_RD = new DBParm();
        DDTBSP11_RD.TableName = "DDTBSP11";
        IBS.DELETE(SCCGWA, DDRBSP11, DDTBSP11_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
            SCCRWBSP.RETURN_INFO = 'N';
