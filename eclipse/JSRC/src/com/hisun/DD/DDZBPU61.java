...
public class DDZBPU61 {
    ...
    DDRBSP61 DDRBSP61 = new DDRBSP61();
    SCCGWA SCCGWA;
    ...
    public void MP(SCCGWA SCCGWA, SCCRWBSP SCCRWBSP) throws IOException,SQLException,Exception {
        ...
        B00_MAIN_PROC();
        if (pgmRtn) return;
        CEP.TRC(SCCGWA, "DDZBPU61 return!");
        Z_RET();
        if (pgmRtn) return;
    }
    ...
    public void B00_MAIN_PROC() throws IOException,SQLException,Exception {
        ...
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DDRBSP61);
        JIBS_tmp_str[1] = JIBS_tmp_str[0];
        IBS.CLONE(SCCGWA, DDRBSP61, DDRBSP1);
        JIBS_tmp_str[0] = IBS.CLS2CPY(SCCGWA, DDRBSP61);
    ...
    public void T000_READ_BSP() throws IOException,SQLException,Exception {
        DDTBSP61_RD = new DBParm();
        DDTBSP61_RD.TableName = "DDTBSP61";
        IBS.READ(SCCGWA, DDRBSP61, DDTBSP61_RD);
    ...
    public void T000_WRITE_BSP() throws IOException,SQLException,Exception {
        CEP.TRC(SCCGWA, "WRITE BEFORE");
        DDTBSP61_RD = new DBParm();
        DDTBSP61_RD.TableName = "DDTBSP61";
        DDTBSP61_RD.errhdl = true;
        IBS.WRITE(SCCGWA, DDRBSP61, DDTBSP61_RD);
        CEP.TRC(SCCGWA, "WRITE AFTER");
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.DBIO_FLG);
        CEP.TRC(SCCGWA, DDRBSP61.BLOB_TR_DATA);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_READ_BSP_UPD() throws IOException,SQLException,Exception {
        DDTBSP61_RD = new DBParm();
        DDTBSP61_RD.TableName = "DDTBSP61";
        DDTBSP61_RD.col = "STATUS ,RT_CODE ,OUT_JRN ,OUT_VCH_NO";
        DDTBSP61_RD.upd = true;
        IBS.READ(SCCGWA, DDRBSP61, DDTBSP61_RD);
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.DBIO_FLG);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
    ...
    public void T000_REWRITE_BSP() throws IOException,SQLException,Exception {
        DDTBSP61_RD = new DBParm();
        DDTBSP61_RD.TableName = "DDTBSP61";
        IBS.REWRITE(SCCGWA, DDRBSP61, DDTBSP61_RD);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else {
        }
    ...
    public void T000_DELETE_BSP() throws IOException,SQLException,Exception {
        DDTBSP61_RD = new DBParm();
        DDTBSP61_RD.TableName = "DDTBSP61";
        IBS.DELETE(SCCGWA, DDRBSP61, DDTBSP61_RD);
        CEP.TRC(SCCGWA, SCCGWA.COMM_AREA.DBIO_FLG);
        if (SCCGWA.COMM_AREA.DBIO_FLG == '0') {
            SCCRWBSP.RETURN_INFO = 'F';
        } else if (SCCGWA.COMM_AREA.DBIO_FLG == '1') {
